...
export function* importPostman() {
    yield takeEvery(ImportPostmanDataType, function* (action: any) {
        const channelAction = syncAction({
            type: ImportPostmanDataType,
            method: HttpMethod.POST,
            url: Urls.getUrl(`collection/postman/${action.value.projectId}`),
            body: action.value.data,
            successAction: () => actionCreator(ReloadType)
        });
        yield put(channelAction);
    });
}
